...
class TerrainDecal {
    constructor(private scene: any, decalTextureUrl: string, decalUvRect: Rect, groundArea: Rect, getY: (x: number, z: number) => number) {
        var widthSegs = 4;
        var heightSegs = 4;
        ...
        var posoff = 0; var normoff = 0; var uvoff = 0;
        ...
        for (var verty = 0; verty < vertHeight; ++verty, pz += pzStep, v += vStep) {
            ...
            for (var vertx = 0; vertx < vertWidth; ++vertx, posoff += 3, normoff += 3, uvoff += 3, px += pxStep, u += uStep) {
                positions[posoff + 0] = px; positions[posoff + 1] = getY(px, pz) + 0.1; positions[posoff + 2] = pz;
                normals[normoff + 0] = 0; normals[normoff + 1] = 1; normals[normoff + 2] = 0;
                uvs[uvoff + 0] = u; uvs[uvoff + 1] = v;
            }
        }
